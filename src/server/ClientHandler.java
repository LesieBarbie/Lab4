...
public class ClientHandler extends Thread {
    ...
    private void processOperation(Object obj) throws IOException, ClassNotFoundException {
        if (obj instanceof StopOperation) {
            ...
            showBalance(obj);
        } else if (obj instanceof ShowClientInformationOperation) {
            showClientInformation(obj);
        } else {
            error();
        }
    }
    ...
    private void showClientInformation(Object obj) throws IOException {
        ShowClientInformationOperation op = (ShowClientInformationOperation) obj;
        int ind = bnk.findMetroCard(op.getSerNum());
        if (ind >= 0) {
            os.writeObject("Client Information: " + bnk.getStore().get(ind));
            os.flush();
        } else {
            os.writeObject("Cannot Find Client Information for Card: " + op.getSerNum());
            os.flush();
        }
    }
